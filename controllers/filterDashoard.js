...
exports.downloadPendingEmiDetails = async (req, res) => {
  try {
    let pendingEmiDetails = await loanModel.aggregate([
      ...
      }, // Filter by emiPending and isActive
      {
        $lookup: {
          from: "usermodels",
          localField: "details.loanPayerDetails.name",
      ...
      },
      {
        $group: {
          _id: {
            ...
            pendingEmiNum: "$loanDetails.pendingEmiNum",
            emiPendingDate: "$loanDetails.emiPendingDate",
            principleAmountCurrentMonth:
              "$loanDetails.instalmentObject.principleAmountCurrentMonth",
            interestAmount: "$loanDetails.instalmentObject.interestAmount",
            totalEmiAmountRoundoff:
              "$loanDetails.instalmentObject.totalEmiAmountRoundoff",
          },
        },
      },
      {
        $project: {
          _id: 0,
          HP_Number: "$_id.loanNumber",
          Name: "$_id.loanPayerName",
          Mobile: "$_id.phoneNumber1",
          pendingEmiNum: "$_id.pendingEmiNum",
          emiPendingDate: "$_id.emiPendingDate",
          Principle_Per_Month: "$_id.principleAmountCurrentMonth",
          Interest_Per_Month: "$_id.interestAmount",
          Total_Per_Month: "$_id.totalEmiAmountRoundoff",
        },
      },
      { $sort: { emiPendingDate: 1 } },
    ]);
    ...
    const columnWidths = Object.keys(pendingEmiDetails[0] || {}).map((key) => ({
      wch:
        Math.max(
          key.length,
          ...pendingEmiDetails.map(
            (item) => (item[key]?.toString() || "").length
          )
        ) + 2,
    }));
